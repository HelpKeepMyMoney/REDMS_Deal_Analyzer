...
- **Deal notes history & website link** — Property Info now includes a dedicated **Notes** section with modal-based note management (add/view/edit/delete) and a sidebar note list showing the note date and preview text. Notes are stored per deal in `notesHistory` (each note has `id`, `text`, `createdAt`, `updatedAt`) and legacy single-field notes are bulk-migrated into first notes. `Web Page` accepts a web address and renders a clickable link in the sidebar that opens in a new tab (auto-prefixes `https://` when protocol is omitted).
