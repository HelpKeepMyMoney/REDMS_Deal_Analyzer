...
- **Deal notes history & website links** — Property Info now includes a dedicated **Notes** section with modal-based note management (add/view/edit/delete) and a sidebar note list showing the note date and preview text. Notes are stored per deal in `notesHistory` (each note has `id`, `text`, `createdAt`, `updatedAt`) and legacy single-field notes are bulk-migrated into first notes. **Web Page** and **Pics & Videos** each accept a URL (stored on the deal as `wedPage` and `picsVideos`); both render a clickable link below the input that opens in a new tab (`target="_blank"` with `rel="noopener noreferrer"`; auto-prefixes `https://` when protocol is omitted). Long URLs are clipped in the sidebar (ellipsis on inputs; compact link labels **Listing** and **Pics & Videos** with full URL in `href`/`title`) so the sidebar does not grow horizontally or overflow with very long addresses.
- **Metric cards** — NOI, Investor Flip Profit, Sell to Retail Investor (ARV), Investor Cap Rate, Rent-to-Price Ratio, Flip Cash-on-Cash, B&H Cash-on-Cash, Investment Required, Cap Rate. Cards turn red when thresholds are missed. Three pass/fail checks (Flip Cash-on-Cash ≥ 25%, B&H CoC ≥ 10%, B&H cash ≤ max TPC) appear above the cards in `DealMetrics.jsx`; they scroll with the page (not pinned) and use short labels on mobile (Flip CoC, B&H CoC, B&H Cash) in a 3-column layout.
...
- **Mobile layout** — Responsive design for phones and tablets (≤900px breakpoint). Deal analyzer pages (`/investor`, `/demo`, `/wholesaler`) use a slide-over sidebar drawer with backdrop, compact header with a always-visible **Modules** menu (Admin / Investor / Wholesaler) for admin and wholesaler users, plus an **Actions** menu (PDF export, profile, sign out), horizontally scrollable tabs, and stacked metric cards. The three deal pass/fail checks (Flip CoC, B&H CoC, B&H cash) scroll with the page (not sticky) and display in a compact **3-column grid** on mobile so all ✓ PASS / ✗ FAIL results are visible without horizontal scrolling. Property Search and Admin pages adapt to single-column layouts. Uses `100dvh`, safe-area insets, and 44px touch targets. Shared via `useMobileLayout` hook, `MobileHeaderActions`, and `SidebarBackdrop` components.

## Deal Management (Admin)

**Admin → Deal Management** provides a card-based view of all deals with:

- **Deal cards** — Mimic property search cards; show offer price, beds/baths/sqft, address, status badge, Deal/No Deal badge, metrics (Rehab Level, Est. Rent, Annual NOI, B&H Cash-on-Cash ROI, Investment Required, Loan Amount), **Notes** (all entries from `notesHistory`, newest first, with date labels; legacy `notes` fallback when history is empty), and Created/Updated dates. Each card links to the deal screen (`/investor?dealId=...`).
...
- **Mobile deal checks layout** — Pass/fail checks in `DealMetrics.jsx` use short mobile labels and a 3-column grid (`REDMS.module.css`) so all three ✓ PASS / ✗ FAIL results fit on screen without horizontal scrolling.
- **Mobile UX refinements** — Module switcher (`AdminDropdown` / `WholesalerModuleDropdown`) moved out of the **Actions** menu and is always visible in the header on mobile as **Modules ▼** (Admin, Investor, Wholesaler, Profile). Deal pass/fail checks no longer use sticky positioning.
...
- **Sidebar property link fields** — **Web Page** (`wedPage`) and **Pics & Videos** (`picsVideos`) in Property Info: URL inputs plus clickable links that open in a new tab (`target="_blank"` with `rel="noopener noreferrer"`). Link color uses the app accent token. Under **Web Page Link**, the visible anchor text is **Listing**; under **Pics & Videos Link**, **Pics & Videos** (full URL remains in `href` and tooltip `title`). Long URLs use ellipsis styling on the inputs (`Field` optional `inputClassName`; `.sidebarUrlInput` in `REDMS.module.css`) and sidebar layout uses `minmax(0, 310px)` / `min-width: 0` / `overflow-x: hidden` on the sidebar column so long strings do not force a horizontal scrollbar.
