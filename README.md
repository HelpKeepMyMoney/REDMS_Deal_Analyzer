...
- **Property notes & website link** — Property Info includes **Notes** and **Wed Page** fields. `Wed Page` accepts a web address and renders a clickable link in the sidebar that opens in a new tab (auto-prefixes `https://` when protocol is omitted).
...
- **Sidebar property link field** — Added **Wed Page** input directly after **Notes** in Property Info. Entered URLs now render as clickable links that open in a new tab (`target="_blank"` with `rel="noopener noreferrer"`). Link color uses the app accent token for contrast on dark backgrounds.
