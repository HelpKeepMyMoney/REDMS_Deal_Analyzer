...
- **Metric cards** — NOI, Investor Flip Profit, Sell to Retail Investor (ARV), Investor Cap Rate, Rent-to-Price Ratio, Flip Cash-on-Cash, B&H Cash-on-Cash, Investment Required, Cap Rate. Cards turn red when thresholds are missed. Three pass/fail checks (Flip Cash-on-Cash ≥ 25%, B&H CoC ≥ 10%, B&H cash ≤ max TPC) appear above the cards and scroll with the page (not pinned on scroll).
...
- **Mobile layout** — Responsive design for phones and tablets (≤900px breakpoint). Deal analyzer pages (`/investor`, `/demo`, `/wholesaler`) use a slide-over sidebar drawer with backdrop, compact header with a always-visible **Modules** menu (Admin / Investor / Wholesaler) for admin and wholesaler users, plus an **Actions** menu (PDF export, profile, sign out), horizontally scrollable tabs, and stacked metric cards. The three deal pass/fail checks (Flip CoC, B&H CoC, B&H cash) scroll with the page (not sticky) and use a compact horizontal row on mobile. Property Search and Admin pages adapt to single-column layouts. Uses `100dvh`, safe-area insets, and 44px touch targets. Shared via `useMobileLayout` hook, `MobileHeaderActions`, and `SidebarBackdrop` components.
...
- **Mobile UX refinements** — Module switcher (`AdminDropdown` / `WholesalerModuleDropdown`) moved out of the **Actions** menu and is always visible in the header on mobile as **Modules ▼** (Admin, Investor, Wholesaler, Profile). Deal pass/fail checks no longer use sticky positioning; on mobile they appear in a compact horizontally scrollable row to save vertical space.
